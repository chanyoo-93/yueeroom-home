...
describe('VariantsService', () => {
  ...
  describe('create', () => {
    ...
    it('중복 SKU로 생성 시 ConflictException을 던진다', async () => {
      mockPrisma.product.findUnique.mockResolvedValue(mockProduct);
  ...
  describe('update', () => {
    ...
    it('다른 변형이 사용 중인 SKU로 수정 시 ConflictException을 던진다', async () => {
      mockPrisma.productVariant.findUnique
        .mockResolvedValueOnce(mockVariant) // findOneOrFail
        .mockResolvedValueOnce({ ...mockVariant, id: 'var-99' }); // sku check
